...
st.set_page_config(page_title="NBA Injury Track", layout="wide")
...
st.markdown(
    ...
    html, body, .stApp, .stMarkdown, .stText, p, div, span, label, h1, h2, h3, h4, h5, h6, li, th, td, input, button, .stSelectbox, .stSlider, .stCheckbox {
    ...
    /* --- Base Slider Styles (Unfilled Track) --- */
    div.stSlider [data-baseweb='slider'] div[class*='track'] {
        background-color: #555555 !important;
    }

    /* =========================================
       COLUMN 1: PRIMARY PLAYER (BLUE)
       ========================================= */
    [data-testid="stColumn"]:nth-of-type(1) div[data-baseweb='select'] > div {
        border-color: #7DADE8 !important;
    }
    [data-testid="stColumn"]:nth-of-type(1) div.stSlider [data-baseweb='slider'] div[role='slider'] {
        background-color: #666666 !important;
    }
    [data-testid="stColumn"]:nth-of-type(1) div.stSlider [data-baseweb='slider'] div[class*='progressBar'] {
        background-color: #666666 !important;
    }
    [data-testid="stColumn"]:nth-of-type(1) input[type='checkbox'] {
        accent-color: #7DADE8 !important;
    }
    /* Force the Streamlit custom checkbox div to be blue when checked */
    [data-testid="stColumn"]:nth-of-type(1) div[data-baseweb='checkbox'] div[class*='checked'] {
        background-color: #7DADE8 !important;
        border-color: #7DADE8 !important;
    }

    /* =========================================
       COLUMN 2: COMPARISON PLAYER (RED)
       ========================================= */
    [data-testid="stColumn"]:nth-of-type(2) div[data-baseweb='select'] > div {
        border-color: #E08080 !important;
    }
    [data-testid="stColumn"]:nth-of-type(2) div.stSlider [data-baseweb='slider'] div[role='slider'] {
        background-color: #666666 !important;
    }
    [data-testid="stColumn"]:nth-of-type(2) div.stSlider [data-baseweb='slider'] div[class*='progressBar'] {
        background-color: #666666 !important;
    }
    [data-testid="stColumn"]:nth-of-type(2) input[type='checkbox'] {
        accent-color: #E08080 !important;
    }
    </style>
    """,
...
st.markdown(
    "<h1><span style='color:#E8E8E8;'>NBA Injury Track:</span> <span style='font-size:0.65em;font-weight:400;color:#B0B0B0;'>"
    "an Injury Predictor Dashboard,</span></h1>"
    "<p style='font-size:1rem;font-weight:400;margin-top:-10px;color:#B0B0B0;'>Get ahead of the game. <span style='color:#909090;'>Be an insider.</span></p>",
    unsafe_allow_html=True,
)
st.caption("This website is based on NBA data from 2016–2025, and not affiliated with any NBA organization.")
...
def render_player_section(
    ...
    container.markdown("---")
    container.subheader("XAI Summary")
    ...
    with container.spinner("Synthesizing patient history and ML outputs via Groq..."):
        ...
        container.markdown(
            f'<div style="background-color:#2A2A2A;border-left:4px solid {chart_color} !important;padding:12px;margin-bottom:8px;min-height:185px;display:flex;align-items:center;font-size:0.95rem;line-height:1.4;color:#E8E8E8;height:185px;">{narrative}</div>',
            unsafe_allow_html=True,
        )
        container.caption(
    ...
    chart = (
        ...
        .properties(background="#121212", padding={"left": 40, "right": 40, "top": 10, "bottom": 10})
        .encode(
            x=alt.X(
                "Outcome:N",
                sort=severity_order,
    ...
    container.caption("Probabilities shown are from an XGBoost model.")

    container.markdown("---")
    container.subheader("Clinical Risks")
...
with controls[0]:
    st.markdown("<span style='color:#2A6F9B;font-weight:600;font-size:1.25rem;'>Primary Player</span>", unsafe_allow_html=True)
    ...
    if selected_player != st.session_state.get("last_primary"):
        st.session_state["last_primary"] = selected_player

with controls[1]:
    st.markdown("<span style='color:#C8102E;font-weight:600;font-size:1.25rem;'>Comparison Player</span>", unsafe_allow_html=True)
    comparison_options = (
    ...
    if comparison_player:
        ...
        if comparison_player != st.session_state.get("last_comparison"):
            st.session_state["last_comparison"] = comparison_player

        saved_minutes = st.session_state.get("comp_minutes", comparison_avg)
        saved_b2b = st.session_state.get("comp_b2b", False)
        ...
        comparison_minutes = slider_col.slider(
            ...
            min_value=0,
            max_value=48,
            value=int(round(saved_minutes)),
            key="comparison_minutes",
        )
        comparison_b2b = b2b_col.checkbox("B2B", value=saved_b2b, key="comparison_b2b")

        st.session_state["comp_minutes"] = comparison_minutes
        st.session_state["comp_b2b"] = comparison_b2b

st.markdown("---")
footer = "Source Code & Data: https://github.com/waitthisisntsteam/NBAInjuryTrack/tree/main"
